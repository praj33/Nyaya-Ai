...
def test_nonce_reuse_protection():
    ...
    # Use it once (should succeed in validation but may fail in processing)
    url = f"http://localhost:8000/nyaya/query?nonce={nonce}"
    payload = {
        "query": "Test query 1",
        "jurisdiction_hint": "India",
    ...
    print("First use (should pass nonce validation)...")
    response1 = requests.post(url, json=payload, headers=headers)
    print(f"First request status: {response1.status_code}")
    
    # Check if it's a nonce error or processing error
    if response1.status_code == 400:
        error_detail = response1.json().get("detail", {})
        if isinstance(error_detail, dict) and error_detail.get("error_code") == "INVALID_NONCE":
            print("Nonce validation failed on first use - this shouldn't happen")
            return False
        else:
            print("First request failed due to processing issues (expected)")
    
    # Use it again (should fail with INVALID_NONCE)
    print("Second use (should fail with INVALID_NONCE)...")
    ...
    if response2.status_code == 400:
        ...
        if isinstance(error_detail, dict) and error_detail.get("error_code") == "INVALID_NONCE":
            ...
            print("Second request failed but not due to nonce error")
            print(f"Error: {error_detail}")
            return False
    else:
        print("Second request didn't fail as expected")
        return False
